...
class Tree
  ...
  def get_successor(root)
    ...
    current_node = current_node.left_node while !current_node.nil? && !current_node.left_node.nil?
    current_node
  end
  ...
  def level_order(&block_arg)
    return_arr = []
    given_block = block_given? ? block_arg : proc { |elem| return_arr << elem.data }
    queue = []
    queue << @root
    until queue.empty?
